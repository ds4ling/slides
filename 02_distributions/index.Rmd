...
---
background-image: url(https://www.jvcasillas.com/media/rstats/memes/rstats_pop_v_sample.png)
background-size: contain
...
p4 <- tibble(x = my_outlier_x) %>% 
  ...
  aes(x = x) + 
  geom_histogram(aes(y = ..density..), 
    binwidth = 0.5, fill = 'lightblue', color = 'black') +
  geom_density(bw = 0.5) + 
...
p5 <- combined_dists %>%
  ggplot() +
  aes(x = val, y = type, fill = type, height = ..density..) + 
